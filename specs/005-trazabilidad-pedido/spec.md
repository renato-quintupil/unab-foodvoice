...
2. **Given** un negocio autenticado, **When** consulta el detalle de cualquier pedido existente, **Then** puede verlo — v1 es mono-local (Principio VIII de la constitución), así que todo pedido pertenece al único negocio y no existe el caso de "pedido de otro negocio".
...
- **FR-004**: Cualquier negocio autenticado DEBE poder consultar el detalle y el historial de cualquier pedido — v1 es mono-local (Principio VIII), así que todo pedido pertenece al único negocio y no hay restricción de pertenencia adicional a la del rol.
- **FR-005**: Al consultar el detalle de un pedido que no existe, o que un cliente consulta sin ser el suyo (FR-003), el sistema DEBE responder de forma indistinguible en ambos casos, sin revelar si el pedido existe.
...
- **SC-003**: Ningún cliente puede acceder al historial de un pedido ajeno: el 100% de los intentos de acceso no autorizado reciben una respuesta que no distingue "no existe" de "no autorizado".
...
- **v1 es mono-local (Principio VIII de la constitución)**: no existe el concepto de "otro negocio" ni una tabla de negocios/locales. Cualquier cuenta con rol `NEGOCIO` gestiona el único local del sistema, así que la restricción de pertenencia de FR-004 se cumple con el control de rol únicamente, sin filtrar por un identificador de negocio inexistente. Si en el futuro se admite más de un local, esta épica necesitará revisarse junto con el resto del catálogo (la misma condición de caducidad que ya declara el Principio VIII).
