...
Ambos extremos son **inclusivos** y se interpretan como **días del calendario en el huso horario de referencia del producto**, `America/Santiago` (spec § Convenciones de interfaz): `from` abarca desde el primer instante de ese día y `to` hasta el último del suyo. Así, `from=to=2026-08-15` devuelve los pedidos de ese día completo, que es lo que una persona espera al escribir la misma fecha dos veces; con extremos exclusivos devolvería cero y parecería un defecto. Los dos parámetros son independientes: solo `from` significa «desde esa fecha en adelante», solo `to` significa «hasta esa fecha», y ninguno de los dos significa «todo el historial».

**El huso de la consulta no es el del almacenamiento.** Toda marca de tiempo se guarda y se devuelve en UTC (§ Formato de fechas), y eso no cambia: lo que se declara aquí es cómo se convierte un **día escrito por una persona** en el intervalo de instantes que se consulta. La conversión ocurre en el servicio, que es el único lugar que conoce el huso de referencia; el esquema compartido solo valida la forma de la fecha. Se descartó interpretar los días en UTC —era la definición anterior de este contrato, revisada al resolver `checklists/ux.md` §CHK026—: para un producto de un solo local en Chile, un pedido hecho a las 22:00 habría aparecido en el reporte del día siguiente, y el administrador vería como «del día 16» algo que ocurrió el 15 a la vista de todos.
