...
Las tareas T122 a T128 dejaron su resultado en
[`verificacion.md`](./verificacion.md). **Las siete están ejecutadas**, incluidas
las tres que exigían una persona: T122 y T123 el recorrido por teclado y a 360
píxeles, y T126 la guía funcional completa —secciones A, B, C y E— con las
esperas reales de 15 y 30 minutos. Con ello quedan verificados los cuatro
criterios que solo se comprueban a mano: **SC-001, SC-007, SC-036 y SC-038**.

Conviene recordar por qué la guía no era un trámite: **dos de esos cuatro
criterios no se cumplían** cuando se auditó el código, y se corrigieron después
—SC-038 en T133 y SC-036 en T134—. Una auditoría contra una lista de condiciones
puede dar por buena una condición cumplida a medias.
...
- [x] T133 Asociar cada mensaje de error a su campo con `aria-describedby` en el formulario de alta (`apps/web/src/app/admin/usuarios/nuevo/formulario-alta.tsx`), en el de edición (`.../[id]/editar/formulario-edicion.tsx`) y en el campo «Nueva contraseña» del diálogo de restablecimiento: hoy el párrafo del error lleva `id="error-<campo>"` que **nada referencia**, de modo que el error queda «suelto en la página» y un lector de pantalla no lo anuncia al enfocar el campo. Solo `/login` lo hace bien y sirve de patrón, per `FR-039(c)`, `SC-038` (partial)
- [x] T134 Importar `MSG_ERROR_INESPERADO` de `packages/shared` en lugar del literal `'No pudimos completar la operación.'` en `apps/web/src/app/login/formulario-login.tsx`, `.../usuarios/nuevo/formulario-alta.tsx`, `.../usuarios/[id]/editar/formulario-edicion.tsx` y `.../usuarios/_components/acciones-usuario.tsx`. No es solo duplicación: el literal recorta «Vuelve a intentarlo en unos momentos», es decir la mitad que dice **qué puede hacer la persona**, que la condición 3 de «mensaje claro» exige, per `Principio II`, `plan: packages/shared como fuente única` (contradicts)
- [x] T135 Mantener abierto el diálogo de confirmación cuando la acción se rechaza y mostrar el error **junto al campo que falla**: hoy `ejecutar` en `acciones-usuario.tsx` captura el fallo sin relanzarlo, `confirmar-accion.tsx` cierra el diálogo igualmente y un restablecimiento rechazado por contraseña fuera de rango presenta su mensaje en la fila del listado, lejos del campo, per `FR-032`, `FR-039(c)`, `spec: dónde se presenta cada mensaje` (partial)
- [x] T136 Actualizar `specs/README.md`, que declara E1 como «implementación no iniciada (0 / 109 tareas)» y describe `tasks.md` como «109 tareas», para que refleje la épica construida y verificada, su recuento real de tareas y la verificación manual que sigue pendiente (T126). Es el mismo defecto que T129 corrigió en `CLAUDE.md`, en el archivo al que `CLAUDE.md` remite como estado del producto, per `plan: fases de entrega` (contradicts)

---

## Phase 9: Convergence

- [X] T137 Completar la anotación de `apps/web/src/components/` en el árbol de `plan.md`, que enumera su contenido requisito por requisito pero nombra tres de los seis componentes: faltan `cerrar-sesion.tsx` (FR-006), `inicio-de-rol.tsx` (FR-031) y `campo.tsx` (FR-039c). Los dos primeros no los alcanza **ningún** artefacto, de modo que el recorrido inverso del Principio III no llega hasta ellos —el mismo caso que T131 encontró con `entrada/`, y la misma lección: una fuente solo encuentra lo que enumera—, per `plan: estructura del proyecto`, `Principio III` (unrequested)
- [X] T138 Declarar en `quickstart.md` § Comprobaciones automáticas que la contraseña `test` de `docker-compose.test.yml` y de `services/api/test/setup.ts` **no es un secreto** —da acceso a una base en memoria, sin volumen, en otro puerto y sin ningún dato real—, o retirarla en favor de una generada al levantarla. Hoy SC-027(c) afirma sin matices que «el repositorio no contiene ninguna contraseña ni valor secreto» y T127 lo dio por comprobado, de modo que quien recorra el criterio al pie de la letra encontrará una contradicción donde en realidad hay una decisión razonable sin declarar, per `SC-027` (partial)
