...
- Q: ¿La paginación de 20 por página de la bandeja del negocio (FR-041) aplica solo a los pedidos en `creado`, o también a los que están en `en_preparacion`? → A: Aplica a ambos combinados: la bandeja pagina de a 20 sobre `creado` y `en_preparacion` juntos, ordenados del más antiguo al más reciente, con un filtro opcional por estado.
- Q: ¿A qué acción aplica el límite de "2 clics o menos" de SC-005, dado que rechazar exige escribir un motivo? → A: A ambas; el conteo de clics no incluye escribir el texto del motivo, solo abrir el diálogo de rechazo y confirmar el envío.
...
- Q: ¿Contra qué precio se compara al confirmar cuando el cliente no recargó su carrito recientemente (FR-028)? → A: Contra el precio que el carrito le mostró al cliente en su última carga (no necesariamente el mismo paso de confirmación); el servidor igual revalida contra el precio vigente dentro de la transacción.
...
- **FR-028**: El sistema DEBE volver a validar, en el instante de la confirmación, que cada producto del carrito sigue activo y disponible y que su precio vigente coincide con el precio que el carrito le mostró al cliente en su última carga. Si algún precio cambió, ese intento NO DEBE crear un pedido; el sistema DEBE actualizar el carrito, informar el cambio en español y exigir que el cliente lo revise y confirme nuevamente.
...
- **FR-038**: El sistema DEBE mostrar al negocio sus pedidos pendientes (`creado`) y en preparación (`en_preparacion`), con sus productos, cantidades, precios y dirección de entrega, en la misma bandeja paginada (FR-041), con un filtro opcional por estado.
...
- **FR-041**: El sistema DEBE paginar la bandeja de pedidos del negocio (`creado` y `en_preparacion` combinados) en páginas de 20, ordenadas de forma estable del pedido más antiguo al más reciente, sin repetir ni omitir pedidos al cambiar de página.
...
- **SC-005**: Aceptar o rechazar un pedido se hace en **2 clics o menos** desde la bandeja del negocio (el conteo no incluye escribir el texto del motivo de rechazo, solo abrir el diálogo y confirmar el envío).
