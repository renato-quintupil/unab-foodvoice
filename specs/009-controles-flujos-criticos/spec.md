...
- **FR-003**: El sistema DEBE permitir a un usuario con rol `ADMINISTRADOR` cerrar administrativamente un pedido en cualquier estado no terminal, llevándolo a un estado terminal fuera del camino normal de transición, para los casos en que forzar la transición normal no resuelva el problema.
- **FR-004**: Cerrar administrativamente un pedido DEBE exigir un motivo en texto libre no vacío, y DEBE registrarse en la misma operación indivisible que el cambio de estado.
...
- **A verificar en el Constitution Check del plan**: forzar la transición normal reutiliza el Principio XII sin modificarlo, pero el cierre administrativo fuera del camino normal (Historia 2) introduce un movimiento que la máquina de estados de negocio no declara hoy como transición entre esos dos estados puntuales. El plan debe determinar si esto se declara como un mecanismo administrativo explícitamente fuera de `packages/shared/src/order-state/machine.ts` (con su propia validación y motivo obligatorio), o si exige una enmienda al Principio XII — no darlo por sentado sin más al llegar a esa fase, aunque la expectativa de partida (según el borrador ya revisado con el usuario) es que no hace falta ninguna enmienda.
