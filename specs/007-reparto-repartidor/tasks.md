...
- [ ] T001 Ejecutar la línea base de `specs/007-reparto-repartidor/quickstart.md` —`pnpm test`,
...
- [ ] T002 [P] Crear la carpeta `apps/web/src/app/repartidor/_components/` vacía, lista para los
...
- [ ] T003 Añadir `deliveryUserId` (nullable, FK a `User`, `onDelete: Restrict`) y `assignedAt`
  ...
  `data-model.md`)
- [ ] T004 Generar con `prisma migrate dev --create-only` el archivo
...
- [ ] T005 [P] Actualizar `packages/shared/src/order-state/machine.ts`: agregar
...
- [ ] T006 [P] Añadir `MSG_SIN_PEDIDOS_DISPONIBLES`, `MSG_PEDIDO_YA_NO_DISPONIBLE`,
  `MSG_REPARTIDOR_YA_TIENE_PEDIDO` y `MSG_PEDIDO_NO_ASIGNADO_A_TI` en
  `packages/shared/src/messages/etiquetas.ts` (según `contracts/shared.md`)
- [ ] T007 [P] Añadir `DeliveryOrderDto = OrderSummaryDto & { customerPhone: string }` en
...
- [ ] T011 [P] [US1] Crear las pruebas fallidas de `GET /delivery/orders/available` en
...
- [ ] T012 [P] [US1] Crear las pruebas fallidas de `PUT /delivery/orders/:id/take` en
...
- [ ] T014 [US1] Añadir `disponiblesParaRepartidor()` en
...
- [ ] T015 [US1] Añadir `tomar(id, repartidorId)` en `services/api/src/orders/orders.service.ts`:
  ...
  T010, T014)
- [ ] T016 [US1] Crear `services/api/src/orders/delivery-orders.controller.ts`:
...
- [ ] T017 [US1] Registrar `DeliveryOrdersController` en
  `services/api/src/orders/orders.module.ts` (depende de T016)
- [ ] T018 [US1] Crear `apps/web/src/app/repartidor/_components/pedidos-disponibles.tsx`: pide
...
- [ ] T019 [US1] Reemplazar `apps/web/src/app/repartidor/page.tsx`: exige sesión de rol
...
- [ ] T020 [P] [US2] Crear las pruebas fallidas de `GET /delivery/orders/current` en
...
- [ ] T021 [P] [US2] Extender `services/api/test/delivery-orders-available.integration-spec.ts`
  (T011): confirmar que la respuesta de `available` **nunca** incluye `customerPhone` (SC-007)

### Implementación de US2

- [ ] T022 [US2] Añadir `enCursoDelRepartidor(repartidorId)` en
...
- [ ] T023 [US2] Añadir `GET /` en `delivery-orders.controller.ts` bajo la ruta `current`,
  delegando a `enCursoDelRepartidor` (depende de T022)
- [ ] T024 [US2] Crear `apps/web/src/app/repartidor/_components/pedido-en-curso.tsx`: pide `GET
  /delivery/orders/current`, y si hay pedido muestra productos, cantidades, dirección y teléfono
  (depende de T023)
- [ ] T025 [US2] Integrar `PedidoEnCurso` en `apps/web/src/app/repartidor/page.tsx`, sobre
...
- [ ] T026 [P] [US3] Crear las pruebas fallidas de `PUT /delivery/orders/:id/release` en
...
- [ ] T028 [US3] Añadir `soltar(id, repartidorId)` en
...
- [ ] T029 [US3] Añadir `PUT /:id/release` en `delivery-orders.controller.ts`, delegando a
  `soltar` con el `userId` de la sesión (depende de T028)
- [ ] T030 [US3] Añadir el botón "Soltar pedido" a
